...
try:
    from xaims import *
    serial = XAIMSRC['aims.executable.serial']
    print 'Serial aims = {0} exists={1} executable={2}'.format(serial,
                                                              os.path.exists(serial),
                                                              os.access(serial, os.X_OK))
    parallel = XAIMSRC['aims.executable.parallel']
    print 'Parallel aims = {0} exists={1} executable={2}'.format(parallel,
                                                                os.path.exists(parallel),
                                                                os.access(parallel, os.X_OK))

    for key in XAIMSRC:
        print '{0} = {1}'.format(key, XAIMSRC[key])
except:
    print 'xaims is not installed.'
...
print
print '-------------- Check for ASE and xaims tools ------------'
tools = ['runxaims.py',
         'xaimssum']
...
for exe in tools:
    ...
    if f:
        ...
        print 'xaims/tools is not on your executable path'
...
psp = os.environ.get('AIMS_PP_PATH', None)
if os.path.isdir(psp):
    print 'Pseudopotential database = ', psp
    print 'It contains ', os.listdir(psp)
else:
    print '*** "%s" is not a directory, please check  $AIMS_PP_PATH'
